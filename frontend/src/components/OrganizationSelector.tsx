  ...
  return (
    <div class="relative">
      ...
      <button
        ...
        class={`flex items-center justify-between gap-2 p-4 bg-elevated hover:bg-elevated border border-base rounded-lg transition-colors text-sm cursor-pointer light:shadow-sm ${props.fullWidth ? "w-full" : ""}`}
        title="Switch organization"
      >
        <div class="flex gap-2 items-center">
          <div class="i-carbon-enterprise w-4 h-4 text-muted-body" />
          <span class="text-body max-w-[150px] truncate">
            {currentOrg()?.name || "Select Organization"}
          </span>
        </div>
        <div
          class={`i-carbon-chevron-down w-4 h-4 text-muted-body transition-transform ${isOpen() ? "rotate-180" : ""}`}
        />
      </button>
      ...
      <Show when={isOpen()}>
        ...
        <div class="absolute top-full mt-2 right-0 w-64 bg-elevated border border-base rounded-lg shadow-xl light:shadow-2xl z-50 overflow-hidden animate-slide-down">
          <div class="p-2 border-b border-base">
            <div class="text-xs font-medium text-muted-body px-2 py-1">
              Your Organizations
            </div>
          </div>
              ...
              {(org) => (
                ...
                >
                  <div class="flex-1 min-w-0">
                    ...
                    <div class="text-xs text-muted-body flex items-center gap-2 mt-0.5">
          ...
          <Show when={organizations().length === 0}>
            <div class="px-3 py-8 text-center text-muted-body text-sm">
